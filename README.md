...
We are currently using postgres. You need to set up a user,
   ...
   * create a database, a user and a role
    ```
    CREATE DATABASE hamal_db;
    CREATE USER hamal_user WITH PASSWORD 'hamal_pass';
    ALTER ROLE hamal_user SET client_encoding TO 'utf8';
    GRANT ALL PRIVILEGES ON DATABASE hamal_db TO hamal_user;
    ALTER ROLE hamal_user CREATEDB;
   ```
   * If PostgreSQL version is 15+
   ```
   \c hamal_db
   GRANT ALL ON SCHEMA public TO hamal_user;
   ```
